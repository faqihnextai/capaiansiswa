<!-- resources/views/public_dashboard.blade.php -->
@extends('layouts.app')
<<<<<<< HEAD
...
@section('content')
    ...
    {{-- Story Card Display (Jika ini digunakan, logika JSnya harus di sini atau di file terpisah) --}}
    {{-- Namun berdasarkan permintaan terakhir, story akan membuka halaman baru.
         Jadi, section ini seharusnya tidak diperlukan lagi di public_dashboard.blade.php
         Jika Anda masih ingin fungsionalitas card, beri tahu saya.
         Untuk saat ini, saya akan menyarankan untuk menghapusnya jika memang beralih ke halaman terpisah. --}}
    {{-- <div id="storyCardContainer" class="hidden bg-white p-8 rounded-xl shadow-lg max-w-3xl mx-auto mb-12">
        <h2 class="text-2xl font-bold text-gray-800 mb-6 text-center">Story Terbaru</h2>
        <div id="storyCarousel" class="relative overflow-hidden rounded-lg mb-4">
            <div id="storySlides" class="flex transition-transform duration-500 ease-in-out">
                {{-- Story slides will be injected here --}}
            {{-- </div>
            <button id="prevStory" class="absolute left-2 top-1/2 transform -translate-y-1/2 bg-black bg-opacity-50 text-white p-2 rounded-full focus:outline-none">&#10094;</button>
            <button id="nextStory" class="absolute right-2 top-1/2 transform -translate-y-1/2 bg-black bg-opacity-50 text-white p-2 rounded-full focus:outline-none">&#10095;</button>
            <div id="storyProgressBars" class="absolute bottom-2 left-0 right-0 flex justify-center space-x-1 p-2">
                {{-- Progress bars will be injected here --}}
            {{-- </div>
        </div>
        <button id="closeStoryCard" class="bg-red-500 hover:bg-red-600 text-white font-semibold py-2 px-4 rounded-lg block mx-auto mt-4">Tutup Story</button>
    </div> --}}
...
@section('content')
    ...
    <div class="bg-white p-8 rounded-xl shadow-lg max-w-3xl mx-auto mb-8">
        ...
        <div class="text-gray-700 leading-relaxed">
            <p class="mb-4">Mata pelajaran Informatika diampu oleh **Faqih Baidowi**. Beliau adalah seorang pengajar yang berdedikasi untuk memperkenalkan dunia komputasi dan teknologi kepada siswa dengan cara yang interaktif dan menyenangkan.</p>
            <p class="mb-4">Dalam mata pelajaran ini, siswa akan belajar berbagai konsep dasar informatika, mulai dari pengenalan perangkat keras dan perangkat lunak, dasar-dasar pemrograman, keamanan digital, hingga etika berinternet. Kami percaya bahwa pemahaman tentang informatika sangat penting di era digital saat ini.</p>
            <p class="mb-4">Tujuan utama mata pelajaran ini adalah membekali siswa dengan keterampilan berpikir komputasi, kreativitas dalam memecahkan masalah, serta kemampuan untuk beradaptasi dengan perkembangan teknologi yang pesat. Pembelajaran akan dilakukan melalui proyek-proyek praktis, diskusi kelompok, dan eksplorasi mandiri.</p>
            <p class="font-semibold">Mari bersama-sama menjelajahi potensi tak terbatas di dunia Informatika!</p>
        </div>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Bagian 2: Form Pencarian Capaian Siswa
            // Data dari Laravel (pastikan ini tersedia dari controller)
            const allStudents = @json($students);
            const allGroups = @json($groups);
            const dummyAchievements = @json($dummyAchievements);
            ...
            // Initial population based on stored role if any
            const storedRole = window.getStudentRole();
            if (storedRole) {
                filterClassSelectDashboard.value = storedRole;
                populateGroupFilterDashboard();
            } else {
                populateGroupFilterDashboard(); // Populate initially even without a stored role
            }
        });
    </script>
>>>>>>> 95a11c496064b353818fb73e12dd2a524320dd26
@endsection
